...
basic.forever(function () {
    ...
    PaValue = Math.round(Math.map(HX711.read(), defaultMinPaValue, defaultMaxPaValue, -100, 100))
    ...
    if (PaValue >= 120) {
        ...
        inhaleTag = false
        fornum2 = 0
    } else if (PaValue < -93 && PaValue >= -159) {
        resetStatus()
        inhaleTag = true
        if (fornum2 == 0) {
            saveTime = nowTime
    ...
    if (PaValue >= 0 && PaValue < 120 && isLongTag == false) {
        fornum = 0
    }
    if (PaValue <= 0 && PaValue < -160 && inhaleTag == false) {
        fornum2 = 0
    }
    while (BtmP) {
        Speed = 0
